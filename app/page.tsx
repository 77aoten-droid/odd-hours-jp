...
function regionFor(story: Story) {
  if (story.source.includes("earthquake.usgs.gov") && story.genre === "地球の動き") return "世界全体";
  ...
  if (story.source.includes("earthquake.usgs.gov")) return "記載された震源周辺";
  return "地域別アクセスは取得できません（英語版Wikipedia全体）";
}

function freshnessFor(story: Story) {
  if (story.source.includes("wikipedia.org")) return "確定済みの日別集計（約2日前）";
  if (story.source.includes("earthquake.usgs.gov")) {
    return story.genre === "地球の動き" ? "直近24時間 vs 直近7日" : "直近24時間";
  }
  if (story.source.includes("eonet.gsfc.nasa.gov")) return "直近30日に登録・現在進行中";
  return "NOAAの直近観測値";
}
  ...
  return (
    <main>
      ...
      <section className="hero" id="top">
        ...
        <p className="lead">
          世界で「いつもより動いた数字」を見つけ、<br />
          なにが・どれくらい・なぜ動いたのかを日本語で読み解きます。
        </p>
      ...
      <section className="today-answer" id="today" aria-labelledby="today-answer-title">
        <div className="today-answer-head">
          <div>
            <span>今日の結論</span>
            <h2 id="today-answer-title">まず、この3つを見ればわかる。</h2>
          </div>
          <p>ニュースの大きさではなく、<strong>昨日や普段との差</strong>が大きかった3項目です。</p>
        </div>
        <div className="answer-grid">
          {stories.slice(0, 3).map((story, index) => (
            <a href={`#story-${index + 1}`} key={story.title}>
              <small>{numberLabel(index)}　{story.icon} {story.genre}</small>
              <h3>{story.title}</h3>
              <p>{story.hook}</p>
              <strong>{story.change} → 理由を読む</strong>
            </a>
          ))}
        </div>
        <details className="all-index">
          <summary>今日の全10項目から選ぶ ＋</summary>
          <nav className="jump" aria-label="今日の10項目">
            {stories.map((story, index) => (
              <a href={`#story-${index + 1}`} key={story.title}>
                <b>{numberLabel(index)}</b><span>{story.title}</span>
              </a>
            ))}
          </nav>
        </details>
          ...
          <article key={story.title} className={`story story-${index + 1}`} id={`story-${index + 1}`}>
            ...
            <div className="story-main">
              ...
              <section className="quick-profile" aria-label={`${story.title}の要点`}>
                ...
                <div><small>データ期間</small><strong>{freshnessFor(story)}</strong></div>
              </section>
